/* ═══════════════════════════════════════════════════════════════════════
   DEMO GUARD · vgartg.ru
   Восстанавливает диагональный водяной знак, если его пытаются отключить
   через панель Elements (override !important-правил, удаление <link>/<style>,
   правка inline-стилей на html/body и т.п.).

   Работает в паре с CSS-блоком "DEMO WATERMARK" в style.css.
   Удалить при сдаче клиенту вместе со ссылкой <script> и CSS-блоком.
   ═══════════════════════════════════════════════════════════════════════ */
(function () {
  if (window.__vgrInit) return;
  window.__vgrInit = 1;

  // Собираем текст по кускам — чтобы Ctrl+F "vgartg" не открывал сразу всё
  ...
  var CSS = [
    'html::before,html::after,body::before,body::after{',
      ...
      'text-transform:lowercase !important;color:#002E5B !important;',
      ...
      'mix-blend-mode:multiply !important;text-shadow:none !important;',
  ...
  function inject() {
    if (styleEl && styleEl.isConnected) {
      // гарантируем, что наш <style> идёт ПОСЛЕДНИМ в head — иначе override победит
  ...
  function pseudoOK(el, pseudo) {
    try {
      ...
      // content в computed выглядит как '"vgartg.ru · ..."' — ищем подстроку
  ...
  function verify() {
    ...
    // если inline-стиль на html/body вдруг сломал нам видимость — почистим
    ...
    var bad =
      !pseudoOK(html, '::before') ||
      !pseudoOK(html, '::after')  ||
      !pseudoOK(body, '::before') ||
      !pseudoOK(body, '::after');

    if (bad) {
      // пересоздаём узел заново, чтобы он встал ПОЗЖЕ любого тампер-стиля в head
  ...
  function boot() {
    ...
    // страховочный интервал на случай, если что-то прошло мимо MO
    setInterval(verify, 1500);

    // повторная проверка при возврате к вкладке (часто там и фиддлят)
